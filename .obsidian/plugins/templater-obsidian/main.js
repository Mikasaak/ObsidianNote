...
var TemplaterSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  add_general_setting_header () {
    this.containerEl.createEl("h2", { text: "常规设置" });
  }
  add_template_folder_setting () {
    new import_obsidian6.Setting(this.containerEl).setName("模板文件夹位置").setDesc("此文件夹中的文件将作为模板可用。").addSearch((cb) => {
      new FolderSuggest(cb.inputEl);
      cb.setPlaceholder("示例：文件夹1/文件夹2").setValue(this.plugin.settings.templates_folder).onChange((new_folder) => {
  ...
  add_internal_functions_setting () {
    ...
    desc.append("Templater 提供了多个预定义的变量/函数，您可以使用它们。", desc.createEl("br"), "查看 ", desc.createEl("a", {
      href: "https://silentvoid13.github.io/Templater/",
      text: "文档"
    }), " 获取所有可用的内部变量/函数列表。");
    new import_obsidian6.Setting(this.containerEl).setName("内部变量和函数").setDesc(desc);
  }
  add_syntax_highlighting_settings () {
    const desktopDesc = document.createDocumentFragment();
    desktopDesc.append("在编辑模式下为 Templater 命令添加语法高亮显示。");
    const mobileDesc = document.createDocumentFragment();
    mobileDesc.append("在移动设备的编辑模式下为 Templater 命令添加语法高亮显示。请谨慎使用：这可能会破坏移动平台上的实时预览。");
    new import_obsidian6.Setting(this.containerEl).setName("桌面端语法高亮显示").setDesc(desktopDesc).addToggle((toggle) => {
    ...
    new import_obsidian6.Setting(this.containerEl).setName("移动端语法高亮显示").setDesc(mobileDesc).addToggle((toggle) => {
  ...
  add_auto_jump_to_cursor () {
    const desc = document.createDocumentFragment();
    desc.append("插入模板后自动触发 ", desc.createEl("code", { text: "tp.file.cursor" }), "。", desc.createEl("br"), "您也可以设置热键手动触发 ", desc.createEl("code", { text: "tp.file.cursor" }), "。");
    new import_obsidian6.Setting(this.containerEl).setName("自动跳转到光标").setDesc(desc).addToggle((toggle) => {
  ...
  add_trigger_on_new_file_creation_setting () {
    const desc = document.createDocumentFragment();
    desc.append("Templater 将监听新文件创建事件，并替换新文件内容中的每个命令。", desc.createEl("br"), "这使得 Templater 与 Daily note 核心插件、Calendar 插件、Review 插件、Note refactor 插件等其他插件兼容。", desc.createEl("br"), desc.createEl("b", {
      text: "警告: "
    }), "如果在创建时创建具有未知/不安全内容的新文件，则可能会造成危险。请确保每个新文件的内容在创建时是安全的。");
    new import_obsidian6.Setting(this.containerEl).setName("新文件创建时触发 Templater").setDesc(desc).addToggle((toggle) => {
  ...
  add_ribbon_icon_setting () {
    const desc = document.createDocumentFragment();
    desc.append("在侧边栏的 Ribbon 中显示 Templater 图标，让您可以快速在任何地方使用模板。");
    new import_obsidian6.Setting(this.containerEl).setName("在侧边栏显示图标").setDesc(desc).addToggle((toggle) => {
  ...
  add_templates_hotkeys_setting () {
    this.containerEl.createEl("h2", { text: "模板快捷键" });
    const desc = document.createDocumentFragment();
    desc.append("模板快捷键允许您将模板绑定到热键。");
    ...
    this.plugin.settings.enabled_templates_hotkeys.forEach((template, index) => {
      const s = new import_obsidian6.Setting(this.containerEl).addSearch((cb) => {
        ...
        cb.setPlaceholder("示例：文件夹1/模板文件").setValue(template).onChange((new_template) => {
          if (new_template && this.plugin.settings.enabled_templates_hotkeys.contains(new_template)) {
            log_error(new TemplaterError("此模板已绑定到热键"));
            return;
          }
        ...
        cb.setIcon("any-key").setTooltip("配置热键").onClick(() => {
          app.setting.openTabById("hotkeys");
          const tab = app.setting.activeTab;
          tab.searchInputEl.value = "Templater: 插入";
          tab.updateHotkeyVisibility();
        });
      }).addExtraButton((cb) => {
        cb.setIcon("up-chevron-glyph").setTooltip("向上移动").onClick(() => {
          arraymove(this.plugin.settings.enabled_templates_hotkeys, index, index - 1);
          this.plugin.save_settings();
          this.display();
        });
      }).addExtraButton((cb) => {
        cb.setIcon("down-chevron-glyph").setTooltip("向下移动").onClick(() => {
          arraymove(this.plugin.settings.enabled_templates_hotkeys, index, index + 1);
          this.plugin.save_settings();
          this.display();
        });
      }).addExtraButton((cb) => {
        cb.setIcon("cross").setTooltip("删除").onClick(() => {
    ...
    new import_obsidian6.Setting(this.containerEl).addButton((cb) => {
      cb.setButtonText("为模板添加新的快捷键").setCta().onClick(() => {
  ...
  add_folder_templates_setting () {
    this.containerEl.createEl("h2", { text: "文件夹模板" });
    const descHeading = document.createDocumentFragment();
    descHeading.append("当在给定文件夹中创建一个", descHeading.createEl("strong", { text: "空白 " }), "文件时，文件夹模板将被触发。", descHeading.createEl("br"), "Templater 将使用指定的模板填充空白文件。", descHeading.createEl("br"), "将使用最深的匹配。全局默认模板将在根目录", descHeading.createEl("code", { text: "/" }), " 上定义。");
    ...
    descUseNewFileTemplate.append("启用后，Templater 将使用下面定义的文件夹模板。");
    new import_obsidian6.Setting(this.containerEl).setName("启用文件夹模板").setDesc(descUseNewFileTemplate).addToggle((toggle) => {
    ...
    new import_obsidian6.Setting(this.containerEl).setName("添加新的").setDesc("添加新的文件夹模板").addButton((button) => {
      button.setTooltip("添加额外的文件夹模板").setButtonText("+").setCta().onClick(() => {
    ...
    this.plugin.settings.folder_templates.forEach((folder_template, index) => {
      const s = new import_obsidian6.Setting(this.containerEl).addSearch((cb) => {
        new FolderSuggest(cb.inputEl);
        cb.setPlaceholder("文件夹").setValue(folder_template.folder).onChange((new_folder) => {
          if (new_folder && this.plugin.settings.folder_templates.some((e) => e.folder == new_folder)) {
            log_error(new TemplaterError("此文件夹已关联模板"));
            return;
          }
        ...
        new FileSuggest(cb.inputEl, this.plugin, FileSuggestMode.TemplateFiles);
        cb.setPlaceholder("模板").setValue(folder_template.template).onChange((new_template) => {
  ...
  add_startup_templates_setting () {
    this.containerEl.createEl("h2", { text: "启动模板" });
    const desc = document.createDocumentFragment();
    desc.append("启动模板是 Templater 启动时执行的模板。", desc.createEl("br"), "这些模板不会输出任何内容。", desc.createEl("br"), "这对于设置在启动时添加钩子到 Obsidian 事件的模板非常有用，例如。");
    new import_obsidian6.Setting(this.containerEl).setDesc(desc);
      ...
      const s = new import_obsidian6.Setting(this.containerEl).addSearch((cb) => {
        ...
        cb.setPlaceholder("示例：文件夹1/模板文件").setValue(template).onChange((new_template) => {
          if (new_template && this.plugin.settings.startup_templates.contains(new_template)) {
            log_error(new TemplaterError("此启动模板已存在"));
            return;
          }
    ...
    new import_obsidian6.Setting(this.containerEl).addButton((cb) => {
      cb.setButtonText("添加新的启动模板").setCta().onClick(() => {
  ...
  add_user_script_functions_setting () {
    this.containerEl.createEl("h2", { text: "用户脚本函数" });
    let desc = document.createDocumentFragment();
    desc.append("此文件夹中的所有 JavaScript 文件将作为 CommonJS 模块加载，以导入自定义用户函数。", desc.createEl("br"), "文件夹需要从 Vault 访问。", desc.createEl("br"), "查看 ", desc.createEl("a", {
      href: "https://silentvoid13.github.io/Templater/",
      text: "文档"
    }), " 获取更多信息。");
    new import_obsidian6.Setting(this.containerEl).setName("脚本文件夹位置").setDesc(desc).addSearch((cb) => {
      new FolderSuggest(cb.inputEl);
      cb.setPlaceholder("示例：文件夹1/文件夹2").setValue(this.plugin.settings.user_scripts_folder).onChange((new_folder) => {
    ...
    if (!this.plugin.settings.user_scripts_folder) {
      ...
      const files = errorWrapperSync(() => get_tfiles_from_folder(this.plugin.settings.user_scripts_folder), `用户脚本文件夹不存在`);
  ...
  add_user_system_command_functions_setting () {
    let desc = document.createDocumentFragment();
    desc.append("允许您创建与系统命令关联的用户函数。", desc.createEl("br"), desc.createEl("b", {
      text: "警告: "
    }), "从不受信任的来源执行任意系统命令可能存在风险。只运行您理解的来自可信来源的系统命令。");
    this.containerEl.createEl("h2", {
      text: "用户系统命令函数"
    });
    new import_obsidian6.Setting(this.containerEl).setName("启用用户系统命令函数").setDesc(desc).addToggle((toggle) => {
    ...
    if (this.plugin.settings.enable_system_commands) {
      new import_obsidian6.Setting(this.containerEl).setName("超时").setDesc("系统命令的最大超时时间（秒）。").addText((text) => {
        text.setPlaceholder("超时").setValue(this.plugin.settings.command_timeout.toString()).onChange((new_value) => {
          const new_timeout = Number(new_value);
          if (isNaN(new_timeout)) {
            log_error(new TemplaterError("超时必须是数字"));
            return;
          }
      ...
      desc = document.createDocumentFragment();
      desc.append("用于执行命令的 shell 二进制文件的完整路径。", desc.createEl("br"), "此设置是可选的，如果未指定，将默认为系统的默认 shell。", desc.createEl("br"), "如果有疑问，您可以在所有平台上使用正斜杠（'/'）作为路径分隔符。");
      new import_obsidian6.Setting(this.containerEl).setName("Shell 二进制文件位置").setDesc(desc).addText((text) => {
        text.setPlaceholder("示例：/bin/bash，...").setValue(this.plugin.settings.shell_path).onChange((shell_path) => {
      ...
      this.plugin.settings.templates_pairs.forEach((template_pair) => {
        ...
        const setting2 = new import_obsidian6.Setting(this.containerEl).addExtraButton((extra) => {
          extra.setIcon("cross").setTooltip("删除").onClick(() => {
          ...
          const t = text.setPlaceholder("函数名称").setValue(template_pair[0]).onChange((new_value) => {
            const index = this.plugin.settings.templates_pairs.indexOf(template_pair);
            if (index > -1) {
          ...
          const t = text.setPlaceholder("系统命令").setValue(template_pair[1]).onChange((new_cmd) => {
            const index = this.plugin.settings.templates_pairs.indexOf(template_pair);
            if (index > -1) {
      ...
      const setting = new import_obsidian6.Setting(this.containerEl).addButton((button) => {
        button.setButtonText("添加新的用户函数").setCta().onClick(() => {
  ...
  add_donating_setting () {
    const s = new import_obsidian6.Setting(this.containerEl).setName("捐赠").setDesc("如果您喜欢这个插件，请考虑捐赠以支持持续开发。");
